<?php
namespace Pragma\Historic\Helpers;
...
class Migrate{
	...
	public static function preUpdate(PackageEvent $event) {
		if($event->getOperation()->getPackage()->getName() == "pragma-framework/historic"){
			if (!self::checkConfig($event)) {
				die();
			}
		}
	...
	public static function postUpdate(PackageEvent $event){
		if($event->getOperation()->getTargetPackage()->getName() == "pragma-framework/historic"){
			if (self::checkConfig($event)) {
				// Run phinx migrate
				self::phinxMigrate($event);
			} else {
	...
	protected static function checkConfig(&$event) {
		// base on ./vendor/pragma-framework/historic/Pragma/Historic/Helpers/ path
